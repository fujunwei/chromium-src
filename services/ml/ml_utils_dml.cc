...
OperandDML::OperandDML(std::vector<uint32_t>& dimensions,
  ...
  switch (dimensions.size()) {
    ...
    case 3:
      ...
      break;
    case 4:
      // Test for weights depthwise convolution.
      if (depthwise) {
        dimensions = {dimensions[0], 1, dimensions[3], dimensions[1],
                      dimensions[2]};
      } else {
  ...
  if (depthwise) {
    strides_[0] = dimensions[1] * dimensions[2] * dimensions[3] *
                  dimensions[4];  // new_n = h * w *c
    strides_[1] = 1;              // new_c = 1
    strides_[2] = 1;
    strides_[3] =
        dimensions[1] * dimensions[2] * dimensions[3];  // new_h = w * c * D
    strides_[4] = dimensions[1] * dimensions[2];        // new_w = c * D
  } else {
    strides_[0] =
