...
namespace ml {
  ...
  MPSCNNConvolution* API_AVAILABLE(macosx(10.13)) CreateMPSCNNConvolution(
      ...
      int32_t type) {
    Class descriptor_class = type == mojom::DEPTHWISE_CONV_2D ?
        NSClassFromString(@"MPSCNNDepthWiseConvolutionDescriptor") :
        NSClassFromString(@"MPSCNNConvolutionDescriptor");
    const MPSCNNConvolutionDescriptor* desc = [descriptor_class
 ...
 bool CompilationImplMac::ParameterExtracterForConv(const OperationMac& operation, 
    ...
    for (size_t i = 0; i < operations_.size(); ++i) {
      ...
      if (type == mojom::CONV_2D || type == mojom::DEPTHWISE_CONV_2D) {
        ...
        if (is_bnns_) {
          ...
          success = CompileFullyConnected(operation);
      } else {
        DLOG(ERROR) << "Operation is not supported";
        success = false;
      }
  ...
  bool CompilationImplMac::CompileFullyConnected(OperationMac& operation) {
    // operation.inputs[0] is the index of input in operands_.
    OperandMac& input = operands_[operation.inputs[0]];
    ...
    int32_t input_size = operands_[inputs[1]].dimensions[1];
    input.dimensions = std::vector<uint32_t>({product(input.dimensions) / input_size,
          input_size});

    if (@available(macOS 10.13, *)) {
      operation.fuse_code = getScalarInt32(values_[inputs[3]], memory_.get());
      ...
      // weights.
      const float* source_weights = reinterpret_cast<const float*>(
          memory_.get() + values_.at(inputs[1]).offset);
      // inputs[2] is index of bias, values_.at(inputs[2]) is value info of
      // bias.
      const float* source_bias = reinterpret_cast<const float*>(
          memory_.get() + values_.at(inputs[2]).offset);
      
      // the output_size is the same as first dimension of weights.
      int32_t output_size = operands_[operation.outputs[0]].dimensions[1];
      operation.mpscnn_kernel.reset(CreateMPSCNNConvolution(1, 1, input_size,
                                                            output_size, 1,
                                                            1, source_weights,
                                                            source_bias, relu,
                                                            operation.type));
